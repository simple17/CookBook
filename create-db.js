  ...
  db.recipes.createIndex({"Id": 1}, {unique: true}, function (err, res) {
    ...
    insertRecipe({
      ...
      Ingredients:[{
        Amount: 2,
        AmountType: "шт.",
        Ingredient: { Name: "Филе куриное", AvgWeight: 500, BE: 0.08, Calories: 137.6, Prot: 20.4, Fat: 5.9, NFattyAcids: 5.28, Carb: 3.73, K: 289, Na: 90, Mg: 24, Ca: 15, VitC: 25, VitB5: 2.62, VitPP: 0, Fiber: 0, Water: 62.65 }
      },
      {
        Amount: 6,
        AmountType: "шт.",
        Ingredient: { Name: "Картофель", AvgWeight: 100, BE: 1.64, Calories: 77, Prot: 2, Fat: 0.4, NFattyAcids: 0.1, Carb: 16.3, K: 568, Na: 5, Mg: 23, Ca: 10, VitC: 20, VitB5: 0.3, VitPP: 1.3, Fiber: 1.4, Water: 78.6 }
      },
      {
        Amount: 2,
        AmountType: "шт.",
      ...
      Title: "Запеканка из курицы со сметаной 'нежность'",
      Image: "/img/3.jpg",
      Instructions: "Куриное филе моем, режем на небольшие кубики. Мелко режем лук, отправляем к филе и перемешиваем. Картофель чистим и нарезаем небольшими кубиками. Добавляем к филе с луком, перемешиваем. Солим, перчим и еще раз перемешиваем. Готовим заливку. Яйца взбиваем вилкой. Добавляем сметану (сметану можно заменить сливками или жирным молоком). Все хорошо перемешиваем. Затем добавляем муку просеянную через сито. И хорошо перемешиваем, чтобы не было комочков. Вливаем нашу заливку в мясо с картофелем и перемешиваем. Смазываем чашу мультиварки сливочным маслом и выкладываем в неё нашу смесь. Сверху посыпаем тертым сыром. Устанавливаем мультиварку в режим 'Выпечка', время приготовления 50 минут. Готовую запеканку немного остужаем, разрезаем на порции и подаем на стол! Приятного аппетита!"
    });
  });
